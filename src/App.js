...
import headImg from './images/character/head.svg';
import bodyImg from './images/character/body.svg';
...
const ShowMyGear = () => {
  ...
  const wearBackpack = useCallback(()=>{
    ...
    const strap = document.getElementById('strap-outline');
      strap.setAttribute("d", strapOutlinePath);
      strap.style.fill = '#000';
    const strapColor = document.getElementById('strap-color');
      strapColor.setAttribute("d", strapColorPath);
      strapColor.style.fill = getRandomColor();
  }, [])

  const wearShoes = useCallback(()=>{
    const shoes = document.getElementById('shoes-outline');
      shoes.setAttribute("d", shoesOutlinePath);
      shoes.style.fill = '#000';
  }, [])
  ...
  return (
    <div>
      ...
      <object type='image/svg+xml' data={headImg} aria-label='jsx-a11y/alt-text'></object>
      <img src={bodyImg} alt='character' />
      ...
      <div>
        <button onClick={wearClothes}>穿衣服</button>
        <button onClick={wearPants}>穿褲子</button>
        <button onClick={wearShoes}>穿鞋子</button>
        <button onClick={wearBackpack}>穿背包</button>
      </div>
    </div>
  );
}
